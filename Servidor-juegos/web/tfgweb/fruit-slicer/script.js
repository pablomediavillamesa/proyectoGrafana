...
$(function () {
  ...
  $("#startReset").click(function () {
    if (playing == true) {
	//if we are playing (si estamos jugando contamos reincios cuando se recarga la pagina
	fetch('/metrics_logger.php?game=fruit&event=start').then(() => location.reload()); //reload page
    } else {
      //if we are not playing from before
      $("#front").hide();
      $("#score").show();
      playing = true;
	fetch('/metrics_logger.php?game=fruit&event=start'); // ← Aquí para contar inicios, si no estamos jugando de antes
      //set score to 0
      score = 0;
    ...
    action = setInterval(function () {
      ...
      if ($("#fruit1").position().top > $("#fruitcontainer").height() - 50) {
        ...
        if (trialsleft > 1) {
          ...
          // enviar metricas a metics logger
           fetch('/metrics_logger.php?game=fruit&event=score&score=' + score);
